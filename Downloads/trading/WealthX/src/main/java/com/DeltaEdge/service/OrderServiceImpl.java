...
public class OrderServiceImpl implements OrderService {
    ...
    public Order buyAsset(Coin coin, double quantity, User user) {
        ...
        try {
            ...
            auditService.log(user.getId(), "BUY_ORDER", "Purchased " + quantity + " " + coin.getSymbol(), "SUCCESS", request);

            return savedOrder;
        } catch (Exception e) {
            auditService.log(user.getId(), "BUY_ORDER", "Failed to purchase " + coin.getSymbol() + ": " + e.getMessage(), "FAILED", request);
    ...
    public Order sellAsset(Coin coin, double quantity, User user) {
        ...
        try {
            ...
            auditService.log(user.getId(), "SELL_ORDER", "Sold " + quantity + " " + coin.getSymbol(), "SUCCESS", request);

            return savedOrder;
        } catch (Exception e) {
            // BANKING AUDIT: Failure Log
            auditService.log(user.getId(), "SELL_ORDER", "Failed to sell " + coin.getSymbol() + ": " + e.getMessage(), "FAILED", request);
            throw new InvalidOrderException(e.getMessage());
        }
    }
